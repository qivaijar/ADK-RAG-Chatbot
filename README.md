...
Please note that this process may take some time to complete (around 1–5 minutes) after issuing the query to update the RAG knowledge base. Kindly wait until the agent confirms that the RAG knowledge base has been updated before sending additional commands.
...
For the evaluation, I used the RAGAS framework to assess the quality of the answers generated by my RAG agent, with an LLM serving as the evaluator (“judge”). The evaluation can be triggered by running the evaluation.py script.
Two metrics were measured in this process, with the latest scores as follows:

1. **Faithfulness**: 0.9526
2. **Answer Relevancy**: 0.6762
